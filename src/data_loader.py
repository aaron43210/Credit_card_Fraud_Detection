"""
data_loader.py — Stage 2: Data Collection & Understanding
============================================================
Loads the IEEE-CIS Fraud Detection dataset, merges transaction and
identity tables, and provides initial dataset statistics.
"""
...
def load_raw_data(sample_frac: float = None):
    """
    ...
    - train_identity.csv:    144,233 rows × 41 columns  (device/network features)

    They are joined on TransactionID via LEFT JOIN — not all transactions
    have identity information (~24% do).

    Parameters
    ----------
    sample_frac : float, optional
        If provided, randomly sample this fraction of the data (for quick testing).
    ...
    if sample_frac is not None and 0 < sample_frac < 1:
        ...
        logger.info(f"  Sampled: {format_number(n_before)} → {format_number(len(df))} rows ({format_pct(sample_frac)})")
...
def get_dataset_summary(df: pd.DataFrame) -> dict:
    """
    Generate comprehensive dataset statistics for documentation.

    Returns
    -------
    dict
        Dictionary with dataset shape, class distribution, missing info, etc.
    """
