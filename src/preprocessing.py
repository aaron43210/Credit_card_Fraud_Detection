...
and train/val/test splitting with stratification.
"""
...
def handle_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    2. Median imputation for numeric columns (<50% missing)
...
def cap_outliers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Fraudulent transactions can have extremely high amounts that
    destabilize model training.
    """
...
def encode_categoricals(df: pd.DataFrame) -> tuple:
    ...
    tuple of (pd.DataFrame, dict)
        ...
        (needed for Streamlit inference).
    """
    label_encoders = {}
...
def split_data(df: pd.DataFrame):
    ...
    Stratification ensures each split preserves the ~3.5% fraud rate,
...
def build_hetero_graph(df: pd.DataFrame, 
                       ...
                       y: pd.Series,
                       train_idx, val_idx, test_idx) -> HeteroData:
    """
    Build a Heterogeneous Graph for Fraud Detection using PyTorch Geometric.

    Nodes:
    - transaction (Features: scaled tabular data)
    - card (Features: dummy)
    - device (Features: dummy)

    Edges:
    - (transaction, used, card)
    - (transaction, origination, device)
    """
    ...
    # Temporal decay from TransactionDT recency (newer tx => higher weight).
    # This mirrors the HGNN-ATT-TD idea without dense NxN matrices.
    ...
    # Helper mapping array of transaction local ids 0...N-1
    tx_indices = np.arange(num_transactions)

    # --- 2. CARD NODES & EDGES ---
    # Convert card1 values into unique contiguous node IDs 
    unique_cards, card_idx = np.unique(df['card1'].values, return_inverse=True)
    num_cards = len(unique_cards)
    data['card'].x = torch.ones((num_cards, 1))  # Dummy initialized features
    
    # Form bipartite edges
    ...
    # Reverse Edge: Card -> used_by -> Transaction
    data['card', 'used_by', 'transaction'].edge_index = card_edge_index.flip([0])
    
    # --- 3. DEVICE NODES & EDGES ---
    # Ensure DeviceInfo exists or fallback to a dummy if highly missing
    if 'DeviceInfo' in df.columns:
        device_vals = df['DeviceInfo'].values
    else:
        # Fallback categorical device substitute if dropped
        device_vals = df['DeviceType'].values if 'DeviceType' in df.columns else np.zeros(num_transactions)
    ...
    logger.info(f"    Graph Extracted: {num_transactions} Txs | {num_cards} Cards | {num_devices} Devices")
    return data
...
def run_preprocessing_pipeline(df: pd.DataFrame):
    ...
    # Reconstruct whole dataframe ordered for HeteroData building
    # So we don't lose positional alignment in the graph
    ...
    # For alignment, we must re-isolate card/device cols from original df based on X_full's sorted index.
