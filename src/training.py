...
STRICT_GPU_ONLY = os.environ.get("STRICT_GPU_ONLY", "0") == "1"
...
def _select_safe_device(prefer_mps: bool = True) -> torch.device:
    if torch.cuda.is_available():
        try:
            ...
            if STRICT_GPU_ONLY:
                raise RuntimeError(
                    f"STRICT_GPU_ONLY=1 but CUDA is busy/unavailable: {exc}"
                ) from exc
            logger.warning(
                f"  CUDA reported available but is busy/unavailable ({exc}). Falling back to CPU."
            )
            return torch.device("cpu")
    if STRICT_GPU_ONLY:
        raise RuntimeError("STRICT_GPU_ONLY=1 but CUDA is not available.")
...
def _neighbor_worker_init(_: int) -> None:
    """Keep each data-loader worker single-threaded to avoid CPU oversubscription."""
...
def _configure_hgnn_runtime_threads() -> None:
    """Bound host-side threading for stable GPU-fed training on shared machines."""
    torch_threads = int(os.environ.get("SAMPLED_HGNN_TORCH_THREADS", "2"))
    interop_threads = int(os.environ.get("SAMPLED_HGNN_TORCH_INTEROP_THREADS", "1"))
    ...
    try:
        ...
        # set_num_interop_threads can fail if parallelism was initialized earlier.
        pass
...
def _build_neighbor_loader(
    ...
    num_workers = int(os.environ.get("SAMPLED_HGNN_NUM_WORKERS", "1"))
    max_workers = int(os.environ.get("SAMPLED_HGNN_MAX_WORKERS", "2"))
    num_workers = max(0, min(num_workers, max_workers))
    persistent_workers = os.environ.get("SAMPLED_HGNN_PERSISTENT_WORKERS", "1") == "1"
    prefetch_factor = int(os.environ.get("SAMPLED_HGNN_PREFETCH_FACTOR", "2"))
    ...
    if num_workers > 0:
        ...
        loader_kwargs["worker_init_fn"] = _neighbor_worker_init
...
def train_neighbor_sampled_hgnn(
    ...
    """Train HGNN with neighbor-sampled mini-batches."""
    logger.info("=" * 50)
    ...
    _configure_hgnn_runtime_threads()

    if STRICT_GPU_ONLY:
        allow_cpu_retry = False

    device = _select_safe_device(prefer_mps=True)
    if STRICT_GPU_ONLY and device.type != "cuda":
        raise RuntimeError(
            f"STRICT_GPU_ONLY=1 requires CUDA device, got {device}."
        )
    ...
    try:
        ...
        if device.type == "cuda" and allow_cpu_retry and _is_cuda_runtime_error(exc):
            logger.warning(
                f"  CUDA runtime error during neighbor-sampled HGNN ({exc}). Retrying on CPU."
            )
...
def train_neural_network(
    ...
    """
    Train the HGNN neural network with Focal Loss and PyTorch Geometric.
    ...
    """
    logger.info("=" * 50)
    logger.info("  🧠 Training Neural Network (HGNN)")
    logger.info("=" * 50)
    ...
    if STRICT_GPU_ONLY and device.type != "cuda":
        raise RuntimeError(
            f"STRICT_GPU_ONLY=1 requires CUDA. Current detected device: {device}."
        )
    ...
    if use_hgnn:
        ...
        if STRICT_GPU_ONLY and device.type != "cuda":
            raise RuntimeError("STRICT_GPU_ONLY=1 does not allow non-CUDA HGNN training.")
    ...
    raise RuntimeError(
        "HGNN-only mode is enabled. Provide hetero_data to train the neural model. "
        "Tabular neural fallback is intentionally disabled in PROJECT package."
    )
...
def train_dense_hgnn_att_td(
    ...
    # Also probe CUDA with a tiny allocation because torch.cuda.is_available() can be True while
    # the selected GPU is busy/unavailable in shared DGX environments.
    if STRICT_GPU_ONLY:
        allow_cpu_retry = False

    device = _select_safe_device(prefer_mps=False)
    if STRICT_GPU_ONLY and device.type != "cuda":
        raise RuntimeError(
            f"STRICT_GPU_ONLY=1 requires CUDA device, got {device}."
        )
    ...
    try:
        ...
        if device.type == "cuda" and _is_cuda_runtime_error(exc):
            if STRICT_GPU_ONLY:
                raise RuntimeError(
                    f"STRICT_GPU_ONLY=1 and CUDA tensor initialization failed: {exc}"
                ) from exc
            logger.warning(
                f"  Failed to initialize CUDA tensors ({exc}). Falling back to CPU for this run."
            )
            device = torch.device("cpu")
            _clear_cuda_state()
            model = model.to(device)
            ...
            adj_list = [adj.cpu() for adj in adj_list]
            dts_list = [dt.cpu() for dt in dts_list]
    ...
    try:
        ...
        if device.type == "cuda" and allow_cpu_retry and _is_cuda_runtime_error(exc):
            logger.warning(
                f"  CUDA runtime error during DenseHGNN training ({exc}). Retrying on CPU."
            )
            os.environ["CUDA_VISIBLE_DEVICES"] = ""
            _clear_cuda_state()
            return train_dense_hgnn_att_td(
                df=df,
                y=y_arr,
